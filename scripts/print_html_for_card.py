...
<body>
	...
	html_content += '''
			...
			other_printings = [];
			related_cards = await fetchRelatedCards();
			for (const c of card_list_arrayified)
			{
	...
	html_content += '''
		...
		async function fetchRelatedCards() {
			let draftmancer_file;
			await fetch(rootPath + `/sets/${card.set}-files/${card.set}-draft.txt`)
					.then(response => response.text())
					.then(text => {
						draftmancer_file = text; 
				}).catch(error => console.error('Error:', error));

			const custom_cards = ('[' + draftmancer_file.split('[CustomCards]')[1].split('\\n\\t[')[1]);
			
			for (const custom_card of JSON.parse(custom_cards)) {
				if (custom_card.name == card.card_name) {
					return custom_card.related_cards || [];
				}
			}
			...
			for (const card_stats of card_list_arrayified) {
				if (card_stats.card_name == card_name && (!set || card_stats.set == set) && (!num || card_stats.number == num)) {
					...
					if ("position" in card_stats) {
						hover_card.src = rootPath + "/sets/" + card_stats.set + "-files/img/" + card_stats.position + ((card_stats.shape.includes("double")) ? "_front" : "") + "." + card_stats.image_type;
					}
					else {
						hover_card.src = rootPath + "/sets/" + card_stats.set + "-files/img/" + card_stats.number + (card_stats.shape.includes("token") ? "t_" : "_") + card_stats.card_name + ((card_stats.shape.includes("double")) ? "_front" : "") + "." + card_stats.image_type;
					}
					return;
				}
			}
